...
b2Body* ModulePhysics::CreateFlipper_l()
{
	b2BodyDef defined;
	defined.type = b2_dynamicBody;
	defined.position.Set(PIXEL_TO_METERS(156), PIXEL_TO_METERS(556));
	ret_flip_l = world->CreateBody(&defined);

	b2PolygonShape box;
	box.SetAsBox(PIXEL_TO_METERS(50) * 0.5f, PIXEL_TO_METERS(10) * 0.5f);

	b2FixtureDef flip_fixture_l;
	flip_fixture_l.shape = &box;
	flip_fixture_l.density = 2;
	ret_flip_l->CreateFixture(&flip_fixture_l);

	b2BodyDef circle_flip_l;
	circle_flip_l.position.Set(PIXEL_TO_METERS(132), PIXEL_TO_METERS(556));
	circle_flip_l.type = b2_staticBody;

	b2Body* circle_body2 = world->CreateBody(&circle_flip_l);
	...
	revoluteJointDef2.bodyA = ret_flip_l;
...
b2Body* ModulePhysics::CreateFlipper_r()
{
	b2BodyDef defined;
	defined.type = b2_dynamicBody;
	defined.position.Set(PIXEL_TO_METERS(220), PIXEL_TO_METERS(556));
	ret_flip_r = world->CreateBody(&defined);

	b2PolygonShape box;
	box.SetAsBox(PIXEL_TO_METERS(50) * 0.5f, PIXEL_TO_METERS(10) * 0.5f);

	b2FixtureDef flip_fixture_r;
	flip_fixture_r.shape = &box;
	flip_fixture_r.density = 2;
	ret_flip_r->CreateFixture(&flip_fixture_r);

	b2BodyDef circle_flip_r;
	circle_flip_r.position.Set(PIXEL_TO_METERS(243), PIXEL_TO_METERS(556));
	circle_flip_r.type = b2_staticBody;

	b2Body* circle_body3 = world->CreateBody(&circle_flip_r);
	...
	revoluteJointDef3.bodyA = ret_flip_r;
